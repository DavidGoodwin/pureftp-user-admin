...
function html_header ($title) {
	...
	<body bgcolor="#ffffff" >
	<table id="secondary-links-table" summary="Navigation elements." border="0" cellpadding="0" cellspacing="0" class="pagetable"><tr>
...
?>
    ...
    <tr>
		...
		<td class="listtd">
			<select name="userinfo[uid]">
				...
				foreach ($a->uids as $key=>$val) {
					?><option value="<?=$key?>" <? if ($userinfo["uid"]==$key) { echo("SELECTED"); } ?>><?=$val?></option><?
				}
				?>
				<option></option>
			</select>
		...
		<?php if(empty($userinfo['gid'])) { $userinfo['gid'] = '65534'; } ?>
		<td class="listtd">
			<select name="userinfo[gid]">
				...
				foreach ($a->gids as $key=>$val) {
					?><option value="<?=$key?>" <? if ($userinfo["gid"]==$key) { echo("SELECTED"); } ?>><?=$val?></option><?
				}
				?>
				<option></option>
			</select>
...
function gen_list ($search = "", $start = 0) {
	...
	<table border="0" cellspacing="0" cellpadding="2"><tr>
		...
		<td class="headertd" width="33%"><div align="center">search</div></td>
		<td class="headertdright">&nbsp;</td>
	</tr><tr>
